...
export function createCalendarModule(config) {
  ...
  async function loadCalendarMonthAppointments(year, month) {
    ...
    let peopleContact = {};
    if (peopleIds.length) {
      const { data: people } = await client.from('profiles').select('id, full_name, email, phone').in('id', peopleIds);
      (people || []).forEach(p => {
        peopleNames[p.id] = p.full_name;
        peopleContact[p.id] = { email: p.email || null, phone: p.phone || null };
      });
    }
    ...
    calendarMonthAppointments = (appts || []).map(a => ({
      ...
      clientEmail: (peopleContact[a.client_id] || {}).email || null,
      clientPhone: (peopleContact[a.client_id] || {}).phone || null,
  ...
  function renderApptRow(a, showPractitioner) {
    ...
    const contactLinks = [
      a.clientPhone ? `<a href="tel:${a.clientPhone}" style="font-size:11.5px;color:var(--primary);margin-right:10px;display:inline-flex;align-items:center;gap:3px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.362 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.338 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>Call</a>` : '',
      a.clientEmail ? `<a href="mailto:${a.clientEmail}" style="font-size:11.5px;color:var(--primary);display:inline-flex;align-items:center;gap:3px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 6l-10 7L2 6"/></svg>Email</a>` : ''
    ].filter(Boolean).join('');
    ...
    <div class="appt-row">
      ...
      <div class="appt-info">
        ...
        ${contactLinks ? `<div style="margin-top:3px;">${contactLinks}</div>` : ''}
      </div>
      ${statusPill}
      ${actionHtml}
    </div>`;
