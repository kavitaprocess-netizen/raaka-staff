...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    //
    // user_metadata here matters — this project already has a
    // handle_new_auth_user() trigger on auth.users that automatically
    // creates the matching profiles row, reading tenant_id/role/full_name
    // straight out of raw_user_meta_data (defaulting role to 'client' if
    // absent). The original bug wasn't the trigger — it's well-designed —
    // it was this call never passing that metadata at all, so tenant_id
    // came through null and hit a real check constraint
    // (tenant_required_unless_admin). Passing it here lets the existing
    // trigger do its job correctly instead of fighting it.
    const createUserResp = await fetch(SUPABASE_URL + '/auth/v1/admin/users', {
      ...
      body: JSON.stringify({
        email: trimmedEmail,
        email_confirm: false,
        user_metadata: { tenant_id: tenantId, role: 'client', full_name: fullName.trim() }
      })
    });
    ...
    // The trigger already inserted the profiles row (id, tenant_id,
    // role, full_name, email) — a second INSERT here would conflict
    // with it. It doesn't set phone at all, so a small follow-up UPDATE
    // handles just that, only if one was actually provided.
    const trimmedPhone = (phone || '').trim();
    if (trimmedPhone) {
      const updateResp = await fetch(SUPABASE_URL + '/rest/v1/profiles?id=eq.' + newClientId, {
        method: 'PATCH',
        headers: {
          'apikey': serviceRoleKey,
          'Authorization': 'Bearer ' + serviceRoleKey,
      ...
      if (!updateResp.ok) {
        ...
        console.error('Phone update error:', updateResp.status, errBody);
        // Not fatal — the account and profile both exist correctly at
        // this point, just without a phone number saved. Worth logging,
